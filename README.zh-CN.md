...
配置流程和主菜单使用同一套全屏 TUI。项目根目录现在通过专门的列表页来新增、编辑和删除，不需要手工拼成一行。macOS 下，光标位于路径输入框时可按 `Ctrl+F` 打开系统文件夹选择器。路径确认后，`gwtm` 会自动检测本机可用的 IDE 启动命令和已安装应用，并让你明确选择一个默认打开方式；你也可以先跳过，等第一次打开 worktree 时再选。
